...
export class MessageList extends Component{
    ...
    makePaymentCall = ( amount, rate ) => {
        ...
        "callbackUrl": "https://www.mobilemoneycad.com/momocad/shop/cart",
        "returnUrl": "https://www.mobilemoneycad.com",
    ...
    render(){
        ...
        if (this.props.messages === null || this.props.messages.length === 0){
            return <h3 className="p-2 text-center text-white text-2xl">
                <MessageLoader {...this.props} />
                {/* No Messages */}
            </h3>
        }
        return (
                ...
                </ConfigProvider>
               {
                 this.props.messages.map(message =>
                    <div className="mt-4 border border-gray-700 rounded-lg shadow bg-gray-800 dark:border-gray-700 m-auto" key={ message.messageId }>
                        <div className="px-5 pb-5 mr-2 py-5">
                            <div className="flex items-center flex-row lg:flex-row md:flex-row justify-between mb-2">
                                <span className="text-xl italic font-semibold tracking-tight text-gray-200 border-dashed p-2 rounded-lg dark:text-white" style={{ fontFamily: 'Roboto'}}>{ message.momocadId  }</span>
                            ...
                            <div className="flex items-center flex-col lg:flex-row md:flex-row justify-between">
                                <h5 className="text-xl text-justify  tracking-tight text-gray-200 dark:text-white">{ (message.body.trim()).toLowerCase().charAt(0).toUpperCase() + (message.body.trim()).substring(1, message.body.length).toLowerCase() }</h5>
                                <button type="button"  className="text-white m-2 ml-2 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" style={{float: "right", backgroundColor: `${ message.category.colour }`}} onClick={ () =>{ 
